...
class TrackingLayer extends PlottingLayer {
  ...
  _createMarkers(lat, lng, isBegin) {
    ...
    fo.setStyle({
      "image": {
        "icon": {
          "src": isBegin ? "@/assets/images/marker-begin.png" : "@/assets/images/marker-current.png",
          "anchor": [0.5, 1]
        }
      }
    });
  ...
  updateTrack(lat, lng) {
    this.track_coordinates.push([lng, lat]);
    if (this.track_coordinates.length < 2) { return; }

    if (!this.fo_end_points) { this.fo_end_points = this._createMarkers(lat, lng); }
  ...
  /**
	 ...
	 */
  getAttribute(key) {
    if (!this.fo_polyline) { return null; }
