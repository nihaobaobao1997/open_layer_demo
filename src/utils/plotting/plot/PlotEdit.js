...
import * as DomUtils from "../util/domutil";
...
class PlotEdit extends Observable {
  ...
  controlPointMouseMoveHandler(e) {
    ...
    if (this.activeControlPointId) {
      ...
      var oldstyle = this.activePlot.getStyle();
      if (plot.type == "mete_windbarb") {
        oldstyle.image_.rotation_ = Math.PI / 2 - PlotUtil.angle(plot.points[0], plot.points[1]);
        this.activePlot.setStyle(new Style({
          geometry: new Point(plot.points[0]),
          image: oldstyle.image_,
          text: oldstyle.text_
        })
        );
      } else if (plot.type == "mete_high_pressure" || plot.type == "mete_low_pressure" || plot.type == "mete_warm_center" || plot.type == "mete_cold_center") {
        oldstyle.image_.rotation_ = Math.PI / 2 - PlotUtil.angle(plot.points[0], plot.points[1]);
        this.activePlot.setStyle(new Style({
          geometry: new Point(plot.points[0]),
          image: oldstyle.image_
  ...
  deactivate() {
    ...
    if (temp_plot) { this.dispatchEvent(new FeatureEvent(FeatureEvent.DEACTIVATE, temp_plot)); }
  }
